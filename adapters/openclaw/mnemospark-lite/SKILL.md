...
When running under OpenClaw, prefer the known-good x402 client flow over hand-built JSON.
...
A known-good production flow used the raw x402 client payload for the paid `/upload` request, retried the same paid request on `202 settlement_pending` with the same payload, then called `/upload/complete` with only `uploadId` and `completion_token`.
